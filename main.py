...
# Train RandomForest (default detection model)
# ------------------------
...
def DetectAction():
    ...
    test = pd.DataFrame([{
        'switch': switch, 'src': src, 'dst': dst, 'pktcount': pktcount,
        'bytecount': bytecount, 'dur': dur, 'flows': flows,
        'pktrate': pktrate, 'Protocol': Protocol, 'tot_kbps': tot_kbps
    }])

    for col in ['switch', 'src', 'dst', 'Protocol']:
    ...
    return render_template("UserApp/Result.html",
                           status=status,
                           Benign=f"{prob[0]:.4f}",
                           Ack=f"{prob[1]:.4f}",
                           PshAck=f"{prob[2]:.4f}")

# Small utility to download dataset example (optional)
